        ...
        HttpRoutes.of[F] {
            case req@ GET -> Root  =>
                val action2 = for {
                    ...
                    result <- userService.getUser(user.legalId).value
                } yield result

                action2.flatMap {
                    case Right(saved) => Ok(saved.asJson)
                    case Left(LegalIDDoesntExistError(existing)) => Conflict(s"The user with legal id ${existing.legalId} doesn't exist")
                }
        }
    }
